...
namespace HttpLib
{
    ...
    {
        abstract class MimeMappingDictionaryBase
        {
            readonly Dictionary<string, string> _mappings = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

            static readonly char[] _pathSeparatorChars = new char[3] { Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar, Path.VolumeSeparatorChar };
            ...
            {
                if (!_mappings.ContainsKey(".*")) AddMapping(".*", "application/octet-stream");
            }
